...
enum ListBetaFeedbackScreenshotsTool {
    static let tool = Tool(
        name: "list_beta_feedback_screenshots",
        description:
            "List screenshot feedback submissions from TestFlight testers. Includes screenshot URLs, device info, tester comments, and build details. Filter by device model, OS version, platform, or build.",
        inputSchema: .object([
            "type": "object",
            "properties": .object([
                "appId": .object([
    ...
    static func handle(
        ...
        typealias FilterAppPlatform = Resources.V1.Apps.WithID
            .BetaFeedbackScreenshotSubmissions.FilterAppPlatform
        ...
        for submission in response.data {
            ...
            // Extract screenshots
            if let screenshots = attrs?.screenshots {
                dict["screenshots"] = screenshots.map { ss -> [String: Any] in
                    var ssDict: [String: Any] = [:]
                    if let url = ss.url { ssDict["url"] = url }
                    if let w = ss.width { ssDict["width"] = w }
                    if let h = ss.height { ssDict["height"] = h }
                    if let exp = ss.expirationDate {
                        ssDict["expirationDate"] = formatter.string(from: exp)
                    }
                    return ssDict
                }
            }
